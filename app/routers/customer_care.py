...
async def verify_customer_care_form(
    ...
    try:
        ...
        result.analysis_id = f"CUSTOMER_CARE_{datetime.now(IST).strftime('%Y%m%d%H%M%S')}"
        ...
        if not hasattr(result, 'error'):
            try:
                ...
                logger.warning(f"Failed to save customer care verification to database: {str(e)}")
                db.rollback()
        ...
        return templates.TemplateResponse(
            "customer_care_output.html",
            {
                ...
                "analyzed_company": company_name,
                "analyzed_phone": phone_number
            }
        )
